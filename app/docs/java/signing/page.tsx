...
export default function Page() {
  return (
    <DocsShell lang="java" section="signing">
      <h1>Signing methods</h1>
      <p>ATick for Java signs with a credential file or with an external key holder (USB token,
      smart-card, HSM, Windows certificate store). Every signing call takes its configuration as a
      single <strong>JSON options string</strong>, and every failure throws
      <code> Atick.AtickException</code> (a <code>RuntimeException</code>).</p>
...
import java.nio.file.*;`} />
      ...
      (<code>.pfx</code> / <code>.p12</code>) and <strong>PEM</strong> — the format is
      auto-detected.</p>
      <Code lang="java" file="SignPfx.java" code={`byte[] pdf = Files.readAllBytes(Path.of("in.pdf"));
...
byte[] signed = Atick.signPfx(pdf, pem,
    "{\\"password\\":\\"\\",\\"cn\\":\\"Axonate Tech\\",\\"pades\\":true}");`} />
      ...
      <p>ATick for Java does not load PKCS#11 libraries or the Windows store itself. To sign with a
      key that never leaves a token, card, HSM, or the OS store, use the <strong>deferred
      flow</strong>: ATick prepares the document and hands you the exact bytes to sign, you produce
      the CMS signature with your own JCA provider (for example <code>SunPKCS11</code>,
...
byte[] signed = Atick.embed(out[0], cms);`} />
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td><code>&quot;tsa_url&quot;</code>, <code>&quot;tsa_auth&quot;</code></td><td>choose / authenticate the timestamp authority</td></tr>
      ...
      encrypted input), and <code>&quot;encrypt_password&quot;</code> /
      <code> &quot;owner_password&quot;</code> (password-protect the output).</p>
...
byte[] v2 = Atick.signPfx(v1,  pfx, "{\\"password\\":\\"••••\\",\\"cn\\":\\"Reviewer\\",\\"pades\\":true}"); // Atick_2`} />
      ...
      <p><a href="/docs/java/pades/">Next page →</a></p>
    </DocsShell>
  );
}
