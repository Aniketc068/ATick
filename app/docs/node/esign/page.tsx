...
export default function Page() {
  return (
    <DocsShell lang="node" section="esign">
      <h1>Indian eSign (CCA)</h1>
      <p>ATick for Node.js supports the CCA eSign Online Electronic Signature Service for
      <strong>every API version</strong> (v1.x … v3.x). The flow is the same across versions — only the
      request XML attributes differ. The same two-step pattern also covers any <strong>remote key</strong>:
      an HSM, USB token, smart-card, or the Windows certificate store.</p>
      <Code lang="node" file="flow.txt" code={`PDF  ->  SHA-256 of the ByteRange (the InputHash, hex)
...
const crypto = require("crypto");`} />
      <p>Every call takes its configuration as a single <strong>JSON options string</strong>, and every
      failure throws an <code>Error</code>.</p>

      <h2>Step 1 — prepare + hash</h2>
      <p><code>atick.prepare</code> returns an object: <code>prepared</code> is the prepared PDF, and
      ...
      <Code lang="node" file="prepare.js" code={`const pdf = fs.readFileSync("in.pdf");
...
const inputHashHex = crypto.createHash("sha256").update(bytesToSign).digest("hex");`} />
      ...
      <p>Put <code>inputHashHex</code> into <code>&lt;InputHash&gt;</code>, then sign the request XML (an
      enveloped W3C XML-DSig) with your own means — your ASP signing key or your ESP&apos;s SDK — and
      POST it to the ESP.</p>
      <Code lang="node" file="request.js" code={`const request =
...
// then POST the signed XML (multipart/form-data) to the ESP.`} />
      ...
      <p>The <code>EsignResp</code> carries the signature in <code>&lt;DocSignature&gt;</code> (Base64).
      Decode it and pass the resulting CMS bytes to <code>atick.embed</code>, together with the prepared
      PDF from step 1.</p>
      <Code lang="node" file="embed.js" code={`const cms = Buffer.from(docSignatureBase64, "base64");  // from <DocSignature>

const signed = atick.embed(prepared, cms);
fs.writeFileSync("signed.pdf", signed);`} />
      <p><code>pkcs7Pdf</code> and <code>pkcs7complete</code> responses already carry the full chain, the
      revocation (under <code>pdfRevocationInfoArchival</code>) and a CA timestamp — so the embedded
      signature is <strong>LTV-complete and timestamped</strong> out of the box.</p>

      <h2>responseSigType</h2>
      <table>
        <thead><tr><th>Value</th><th>Returns</th><th>Embed with</th></tr></thead>
        <tbody>
      ...
      <p>Request a <code>pkcs7Pdf</code> or <code>pkcs7complete</code> reply so the embedded signature is
      LTV-complete.</p>
      ...
      sign <code>bytesToSign</code> directly with your own signer and produce a <strong>detached CMS /
      PKCS#7 SignedData</strong>:</p>
      <ul>
      ...
      <Code lang="node" file="remote-key.js" code={`const { prepared, bytesToSign } = atick.prepare(pdf, JSON.stringify({
  cn: "DS TEST",
  reason: "Approved",
  pades: true
}));
...
fs.writeFileSync("signed.pdf", signed);`} />
      ...
      document. ATick owns the PDF structure; your signer owns the private key.</blockquote>
      ...
      <Code lang="node" file="simulate.js" code={`const pfx = fs.readFileSync("signer.pfx");
...
fs.writeFileSync("signed.pdf", done);`} />
      ...
      <p><a href="/docs/node/api/">Next page →</a></p>
    </DocsShell>
  );
}
